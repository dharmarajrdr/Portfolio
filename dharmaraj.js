$(document).ready(function () {
    ...
    var socialColor = ['#ea4335', '#ebb03d', '#0e76a8', '#00acee', '#4fce5d'];
    const zoho_joining_date = new Date('05/04/2022').getFullYear(),
        currentDate = new Date().getFullYear();
    $('#years_of_experience').html(Math.abs(zoho_joining_date - currentDate));
    for (let i = 0; i < 8; i++) {
        const skills_percentage = $(`.skills_percentage:eq(${i})`).html().replace('%', '');
        $('.progressBar:eq(' + i + ')').css('width', skills_percentage + '%');
    ...
    $('.color').click(function () {
        ...
        $('#greetName span').css('color', $(this).css('color'));
